...
A deployment MAY claim only levels whose REQUIRED behaviors it implements in full; a public validator, where one exists, is how a claim becomes VERIFIED, and until then every claim is the self-declaration this section already names. Self-badge claims beyond implemented behavior are non-conformant marketing, not protocol failure of the wire format. The specification is the standard and a validator IMPLEMENTS it: a validator MUST NOT impose requirements absent from this text. A versioned normative conformance suite (schemas, canonical examples, walk definitions) is committed §19 gate work, so that the suite serves the text rather than quietly becoming it (round-3 flagship finding). And one sentence a client must never read past: EVERY Say artifact — descriptor, levels, answer, record — is the ORIGIN'S OWN STATEMENT, never an observed fact. Conformance proves consistency, not existence: it cannot prove a business is real, competent, or lawful; the descriptor's `levels` field is a self-declaration until a public validator's record says otherwise; and a client MUST NOT present conformance as verification of anything beyond wire behavior (the four-seat kill pass, 2026-08-16).
...
/say/v0/ask                 ask (GET primary; POST required for L1)
/say/v0/facts.json          derived public facts (optional L0/L1)
/say/v0/changes.json        change feed (L3)
/say/v0/r/<id>              record fetch (L2)
```

**Discovery order for consumers:** (1) HTTP `Link` / HTML `link` with `rel="service-desc"` on the human origin, then (2) if that fails, a single GET of the conventional descriptor path `/say/v0/index.json` on the same origin. Step 2 is a fallback, not a new registry. v0 still does not mint `/.well-known/*`. And the fallback cannot be fooled by a soft 404: a document is a Say descriptor ONLY if it parses as JSON carrying `v: 0` and an `endpoints` object — anything else at that path, whatever its status code, is NOT a descriptor and the surface is ABSENT (round-4 fold, DeepSeek — a shared host's 200-HTML 404 page must never read as a surface).
...
| `grain` | the site's OWN declared geographic decision vocabulary (regions, cities, zips, radius classes — whatever the business actually decides by; there is no global closed enum, because real businesses do not decide by a spec's list). The descriptor MUST also declare the MATCH RULE for that vocabulary (the reference serves `geo.match: "slug_exact_or_prefix"`): a declared vocabulary without its matching rule is not conformant. And v0 is honest about the boundary: declaring a rule removes ambiguity only for clients that understand it — v0 defines no universal semantics for site-specific rules, a client MUST NOT infer or approximate a rule it does not understand (it reads those geographic answers as `indeterminate`, §7.1), and a declared rule MUST be specified well enough that an independent client can implement it without consulting the origin's operator; implementations SHOULD use stable, documented rules that others can implement without proprietary software (round-3 fold + re-vote patch; the global enum was argued and declined — see the disposition). And v0 DEFINES the tokens its own text names, so citing them never invites two machines (the fourth-seat kill passes converged here — the two editors' own drafts disagreed on prefix direction until the wire settled it). `slug_exact_or_prefix`: the declared vocabulary is lowercase ASCII slugs (`[a-z0-9]` runs separated by single hyphens); the ask value is normalized first (Unicode compatibility-decomposed, lowercased, every maximal run of characters not in `[a-z0-9]` collapsed to one hyphen, edge hyphens stripped — round-4 precision, Meta), then matches a listed token exactly, or else the first listed token that begins with the normalized ask PLUS `-` — an ask may be a token-boundary prefix of a listed token, never the reverse and never a partial word: asked `pine-hollow`, listed `pine-hollow-valley`, match (`pine-hollow-` is the boundary); asked `pin`, no match, because a slug prefix without a token boundary is not a slug prefix; asked `austin-south`, listed `austin`, no match. Declaration order breaks ties. `repeat_or_csv`: a GET `need` arrives as repeated keys or as one comma-separated value, and an origin advertising the token MUST accept both |
...
| `valid_until` | a date after which this surface's artifacts are EXPIRED — and expiry is never silent. For a 30-day grace window after `valid_until`, an origin MAY keep answering: the descriptor MUST stay HTTP 200 and MUST carry `stale: true`, every response body served under `/say/v0/*` in the window MUST carry the same flag, every response in the window MUST carry `Cache-Control: no-cache` or stricter so no conformant cache serves the stale state, and a consumer MUST NOT present a stale artifact as fresh. `stale` is a boolean, on the descriptor and on answers alike, and it MUST be ABSENT outside the grace window — never `false`, so its very presence is the signal. Past the window, the origin SHOULD serve **410 Gone on the descriptor**; a host that cannot emit 410 MUST serve 404 and MUST NOT serve a 200 — consumers treat either non-200 as ABSENT after expiry, and the forbidden thing was always the stale 200, not the status flavor (round-4 fold, Meta, by our own shared-hosting-floor law). An abandoned install still cannot serve dead facts forever; a lapsed calendar no longer silently erases a correctly configured shop (re-ruled at the 2026-08-16 editors' sitting on a two-house round-2 convergence, Gemma 4 26B + Kimi K2.6, superseding round 1's absolute-ABSENT fold; the reference implementation's self-404 changes to follow this ruling) |
...
| `content` | for the brake light, v0 MUST declare `"same_for_all"`: tiers may change rate, never truth (§11), and a v0 origin MUST NOT vary a verdict or a refusal's `basis` by tier (round-4 fold, Meta — the SHOULD was Principle 2's loophole) |
...
**The ask travels two ways, one meaning.** An origin implementing L1 MUST accept both GET and POST for the ask operation. GET is the cacheable form: parameters ride the query string. POST is the non-query-string form — `Content-Type: application/json`, a JSON object carrying exactly the fields above — and MUST have identical fields, semantics, and verdicts: nothing about an answer may differ by verb. Clients SHOULD prefer POST when the ask may be sensitive, including asks combining a specialized job with `where` or `urgency` (§17 names why: query strings land in logs, histories, and referrers). The MUST is the METHOD on the ask endpoint, not a cross-origin obligation: §16's CORS posture is unchanged, and a browser's cross-origin POST may still be refused at preflight. The downgrade trap that refusal creates is named and closed: a client that judged an ask sensitive enough for POST MUST NOT resend that same ask over GET merely because cross-origin POST is unavailable to it — it uses a non-browser path or does not ask (kill pass, the fourth seat's browser thought experiment). (Round-3 flagship finding; upgraded SHOULD → MUST on the re-vote, ruled by both editors — the floor worth protecting is headers, not verbs, because a host that answers a parameterized GET already runs a dynamic endpoint.)

Unknown or missing `job` → HTTP 400 whose problem document names the rejected `param`. It MUST NOT echo the whole job vocabulary back: the mandatory descriptor fetch already taught it, and a tiny GET forcing a large dynamic reply is an amplification vector (round-4 fold, Gemini — supersedes the teach-in-one-round-trip echo).

Errors are `application/problem+json` (RFC 9457): `type`, `title`, `status`, plus `param` naming the rejected field and that field's allowed vocabulary. `type` is an RFC 9457 URI reference; on the reference surface it is an absolute-path reference under the origin's error path, for example `/say/v0/e/bad-job`. A client MUST treat it as an opaque identifier to match on and MUST NOT dereference it: no origin is obliged to serve a document there, and the reference surface serves none. The descriptor declares the error identifiers an origin can emit, exactly as it declares every other vocabulary — it declares the BARE identifier (`bad-job`), the wire `type` is an origin-relative URI reference whose FINAL PATH SEGMENT is that identifier, and a client matches on that final segment — no second mapping layer, no dereference, no descriptor-vs-`type` string comparison (round-3 sitting). This is law for EVERY origin, not a description of the reference: whatever path an origin roots its errors at, the `type`'s final path segment, percent-decoded, MUST equal a declared identifier (round-4 fold, DeepSeek). Error documents are never cached, and a client MUST NOT read one as a verdict: an error is not a `no`.
...
Answers MUST be served as `Content-Type: application/json` and MUST carry an explicit `Cache-Control` (the reference surface sends `public, max-age` on verdicts and `no-store` on error documents); clients MUST honor the applicable HTTP cache directives and MUST NOT reuse a verdict once it is no longer fresh under them. The protocol-level `stale: true` flag takes PRECEDENCE over HTTP freshness: an answer marked `stale` is never presented as fresh, whatever an intermediary would permit (round-3 fold; precedence from the re-vote, GPT-5.6 Luna). And the precedence is now WIRE-ENFORCEABLE rather than an obligation caches cannot see: an origin MUST NOT emit a `max-age` or `s-maxage` extending past the descriptor's `valid_until` absolute time — a transport cache must never outlive the facts (round-4 fold; Gemini + DeepSeek converged, with Mistral finding the same seam).
...
- A machine answer MUST NOT contradict the CURRENT human-visible substance on the same origin. Parity now has a defined failure mode: where the machine surface and the human substance materially disagree, the origin MUST treat the machine answer as invalid until reconciled, and a client that independently detects the contradiction treats the answer as `indeterminate` and MUST NOT present it as authoritative. A contradiction is MATERIAL when it could change the protocol answer or a reasonable decision based on it — a changed service area is material, a rephrased tagline is not (round-3 re-vote patches, GPT-5.6 Luna)  
...
- Unknown job 400: 93 as measured WITH the vocabulary echo, before round 4 forbade it — re-measure smaller  
...
- All protocol endpoints MUST be served over HTTPS with a valid, publicly trusted certificate, TLS 1.2 or higher. A consumer that can only reach an answer surface over plain HTTP — or over a connection with an invalid, self-signed, or untrusted certificate — MUST treat that surface as ABSENT, not merely degraded, and MUST NOT report its answers as Say answers (cert and version floor: round-4 fold, Mistral)
...
- Fail closed when misconfigured — and an internally unusable descriptor IS misconfiguration: a descriptor missing required declarations or contradicting itself makes the surface ABSENT to a consumer, one rule, no partial reads (round-3 re-vote, GPT-5.6 Luna)  
...
- Action pointers returned in answers MUST be same-SITE with the human origin (same registrable domain) and SHOULD be same-origin exactly (scheme, host, port) — the www-vs-apex reality — or `tel:` URIs; a consumer MUST reject cross-site pointers (round-4 fold, Meta)  
...
- Repetition is its own disclosure: polling any answer over time reveals staffing, disruption, and operating patterns that no single answer shows. "Public on the human site" never means "risk-free to machine-poll," and this specification does not pretend to have prevented it — origins weigh their §11 rate posture with that in mind (kill pass, named cost)
...
3. **Shadow data** exists from at least one real deployment (callers + measured walks), which SHOULD include at least one consumer implementation maintained outside the drafting yard (round-4, DeepSeek, at Sledge's cut)  
...
**2026-08-16, last line of the day:** the fourth seat sat the wave and concurred — no reversal, no new capability, no hidden block. Hostile review made Say smaller. (Verbatim: `ratifications/2026-08-16-chatgpt-gpt-5-6-luna-openai-7-fourth-seat-concur.md`.)

**2026-08-16, the wider council's first round lands — five flagship houses, five ratify-with-patches, zero blocks, and eleven-plus-two sentences fold.** Gemini (Extended Thinking), Meta AI (Muse Spark 1.1), Qwen3.8, DeepSeek, and Mistral Medium 3.5, every seat pasted by the Captain at each house's best free tier, every response verbatim in `ratifications/`. The folds, all sentence-sized, none a capability: `same_for_all` SHOULD→MUST with no tier-varied verdict or basis (Meta); the slug alphabet made exact (Meta); `need_unknown` normative (Meta+Mistral); error final-segment matching made law for every origin (DeepSeek); the stale×cache seam closed wire-enforceably — max-age never outlives `valid_until`, and every grace-window response carries `no-cache` (Gemini+DeepSeek converged, Mistral found the same seam and proposed weakening instead — declined); the bad-`job` 400 loses its vocabulary echo (Gemini's amplification finding — the descriptor already teaches); action pointers same-SITE with same-origin SHOULD (Meta's www-vs-apex catch); idempotency scoped per origin per operation (Meta+DeepSeek); stale scope pinned to `/say/v0/*`; 410-or-404 after expiry so shared hosts stay honest — the forbidden thing was always the stale 200 (Meta, by this spec's own hosting-floor law); `repeat_or_csv` becomes v0's ONLY encoding token (DeepSeek's closed-vocabulary finding at Sledge's single-token cut); §3's own circularity fixed (claims = implemented behavior; validators verify; statement-never-truth already made every claim a self-declaration); and §19 gate 3 SHOULD include an independently maintained consumer. Named in the open rather than folded: FOUR houses attacked exactly the interoperability question the fourth seat left as prey — origin-specific shapes — and the editors still will not mint a schema registry in v0; it stands as the known remaining interoperability cost, first agenda item of any v1 conversation. Also held for their own sittings: L2 record schema, L3 feed identifiers and pagination, the timezone field, record retention depth, and the civil-rights-audit exemption that collided head-on with the audit-as-cover harm (both filed; counsel ground). Both live implementations follow the folds in the same push.

**2026-08-16, the boundary sentence — the kill pass closes.** Sledge's countersign added the last word to the token definition: a slug prefix matches only at a token boundary (ask plus `-`), so `pin` never reaches `pine-hollow-valley`. The reference matcher is corrected to follow the spec — the code follows the ruling, and the change is named, not silent. With that, all four seats' kill jobs are closed: converged blind, folded small, nothing expanded.

**2026-08-16, the fourth seat's second kill job — Sledge's blind pass.** Run without reading Beacon's (the Captain's blind-first order; convergence is signal), against the spine already carrying Beacon's folds. Verdict: the founding promise holds, three at-scale abuse scenarios earn NO new machinery (the wire cannot prove a shop is real, purpose must never gate truth, and a lit surface is enumerable by design — all named, none pretended away), the browser-POST answer is already unambiguous, and a standing refusals list is on the record: identity, signatures, purpose-gating, CORS MUST, descriptor caps, global vocabularies, emergency anything — proposals for those are proposals for a different protocol. One real hole found and closed: the spec NAMED `slug_exact_or_prefix` and `repeat_or_csv` without defining either — and the two editors' own drafts disagreed on prefix direction, proving the hole between the very people who built it. Both tokens are now defined in §6, direction settled by what the reference wire actually serves (an ask may be a prefix of a listed token, never the reverse). Zero new capabilities, again.

**2026-08-16, the four-seat kill pass — Luna takes a seat, and the last round before the wider council makes the spec smaller-minded, not bigger.** After hearing the origin story, the flagship reviewer accepted the Captain's invitation to sit as the fourth reviewer alongside the two drafting crews, and set the exercise: do not add a capability — try to kill Say without expanding it, and if a fix needs a new capability, ask whether it belongs in Say at all (charge filed verbatim in `ratifications/`). Beacon's kill job found five one-sentence fixes and one named cost, all folded, zero new fields: §3 the statement-never-truth rule (conformance proves consistency, not existence; `levels` is self-declaration — closing both the badge upgrade and the ghost-fleet scenario); §7.1 `need` shapes are origin-specific (our own two implementations differ, both conformant — a client never assumes shape across origins); §7.1 urgency reads on the origin's clock; §12 binds consumers equally (the "I only call" escape hatch closed); §7.1 the sensitive-ask GET-downgrade trap closed (POST-unavailable never licenses resending over GET); §17 temporal inference named as a cost this spec does not pretend to prevent. Sledge's kill job remains open on his own lamp. Scope discipline is now itself part of what the record protects.

**2026-08-16, re-vote round 2 (same seat): the cache trap, and four refinements.** The flagship's second re-review (verbatim: `ratifications/2026-08-16-chatgpt-gpt-5-6-luna-openai-3-revote2.md`) reported the prior patches missing from the spine; the live file disproves it — its quotes match the pre-push text exactly, so a browsing cache served the earlier fetch. Vote either way: ratify with patches, fatals none, "I would lift the block." The genuinely new findings landed the same hour, credited: §7.4 defines MATERIAL by consequence (a contradiction matters when it could change the answer or a reasonable decision — a changed service area is material, a rephrased tagline is not); §6 types `stale` as a boolean present ONLY inside the grace window, descriptor and answers alike, absent otherwise so presence is the signal; §6's match rule must be implementable by an independent client without consulting the operator; §16's fail-closed names an internally unusable descriptor as misconfiguration — the surface is ABSENT, one rule; and §7.6's history defense is trimmed to a pointer, because a normative section should not argue its own past (raised twice, taken). Process lesson folded too: re-review asks now carry cache-busted URLs.

**2026-08-16, the re-vote: the BLOCK LIFTS.** The blocking flagship re-reviewed the revised text in the same seat, the same day. Vote: **ratify with patches**; fatal flaws: **none** — its words: the descope "closes the exact escape hatch I blocked on," §7.6 is now "appropriately boring," and recording that the block was not negotiated away is "exactly how a serious ratification process should look" (verbatim: `ratifications/2026-08-16-chatgpt-gpt-5-6-luna-openai-2-revote.md`). Its four remaining patches all landed the same night, both crews signed:
1. §7.4: parity gains a defined FAILURE MODE — on material machine-vs-human disagreement the origin treats the machine answer as invalid until reconciled, and a client detecting the contradiction reads `indeterminate`.
2. §7.2: precedence pinned — `stale: true` beats HTTP freshness, whatever an intermediary would permit.
3. §6: the interoperability boundary stated honestly — a declared match rule removes ambiguity only for clients that understand it; unknown rules are never inferred or approximated; stable documented rules SHOULD be used.
4. §7.1: POST upgraded from SHOULD to **MUST for L1** — if POST is the privacy form, a conformant origin cannot refuse it. Ruled by both editors reversing their own earlier SHOULD: the shared-hosting floor worth protecting is headers, not verbs. The MUST is the method, not a CORS obligation; §5.2's layout line follows (`POST required for L1`).
One nit also taken: §7.6 no longer calls v0 "a complete standard" while the conformance suite remains gate work. The reserved `safety` token stays (cheap, and no wire break if a profile ever comes).
